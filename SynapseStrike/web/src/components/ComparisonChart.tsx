...
const TIME_PERIODS = [
  { key: '1d', hours: 24, label: { en: '1D', zh: '1days' } },
  { key: '3d', hours: 72, label: { en: '3D', zh: '3days' } },
  { key: '7d', hours: 168, label: { en: '7D', zh: '7days' } },
  { key: '30d', hours: 720, label: { en: '30D', zh: '30days' } },
  { key: 'all', hours: 0, label: { en: 'All', zh: 'all' } },
]
...
export function ComparisonChart({ traders }: ComparisonChartProps) {
  ...
  const getTraderCurrentPnl = (traderId: string): number => {
    if (!displayData || displayData.length === 0) return 0
    for (let i = displayData.length - 1; i >= 0; i--) {
      const pnl = displayData[i]?.[`${traderId}_pnl_pct`]
      if (pnl !== undefined) return pnl
    }
    return 0
  }
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <div className="text-sm mt-4 font-medium" style={{ color: '#9CA3AF' }}>
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20">
        <div className="w-20 h-20 rounded-2xl flex items-center justify-center mb-4"
          ...
          <BarChart3 className="w-10 h-10" style={{ color: 'var(--primary)', opacity: 0.6 }} />
        </div>
        <div className="text-lg font-bold mb-2" style={{ color: '#F9FAFB' }}>
          {t('noHistoricalData', language)}
        </div>
        <div className="text-sm text-center max-w-xs" style={{ color: '#9CA3AF' }}>
                ...
                <div key={trader.trader_id} className="flex items-center justify-between gap-4">
                  ...
                  <div className="text-right">
                    <div className="text-sm font-bold mono flex items-center gap-1"
                      style={{ color: isPositive ? 'var(--primary)' : '#F6465D' }}>
                      {isPositive ? <TrendingUp className="w-3 h-3" /> : <TrendingDown className="w-3 h-3" />}
                      {isPositive ? '+' : ''}{pnlPct.toFixed(2)}%
                    </div>
                    <div className="text-[10px] mono" style={{ color: '#6B7280' }}>
                      ${equity?.toFixed(2)}
                    </div>
                  </div>
                </div>
    ...
    <div className="space-y-4">
            ...
            <button
              ...
              style={{
                background: selectedPeriod === period.key
                  ? 'var(--primary-bg, 0.2)'
                  : 'rgba(43, 49, 57, 0.5)',
                color: selectedPeriod === period.key ? 'var(--primary)' : '#9CA3AF',
                border: `1px solid ${selectedPeriod === period.key ? 'var(--primary-bg, 0.4)' : 'rgba(255, 255, 255, 0.08)'}`,
              }}
            >
              {false ? period.label.zh : period.label.en}
            </button>
          ))}
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {traderStats.slice(0, 3).map((trader, idx) => (
            <div key={trader.trader_id}
              ...
              style={{
                background: idx === 0 ? 'var(--primary-bg, 0.15)' : 'rgba(43, 49, 57, 0.5)',
                border: `1px solid ${idx === 0 ? 'var(--primary-bg, 0.3)' : 'rgba(255, 255, 255, 0.08)'}`
              }}>
              ...
              <span className="text-xs font-bold mono"
                style={{ color: trader.currentPnl >= 0 ? 'var(--primary)' : '#F6465D' }}>
          ...
          >
            <defs>
              {traders.map((trader) => {
                ...
                const gradientColor = currentPnl >= 0 ? 'rgb(0, 200, 5)' : 'rgb(255, 80, 0)'
                return (
                  <linearGradient
            ...
            <CartesianGrid strokeDasharray="3 3" stroke="rgba(22, 27, 34, 0.88)" vertical={false} />
                    ...
                    {filteredPayload.map((entry: any, index: number) => {
                      ...
                      return (
                        <div key={`legend-${index}`} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                          ...
                          <span style={{ color: '#F9FAFB', fontSize: '12px', fontWeight: 500 }}>
                            {entry.value}
                            <span style={{
                              color: pnl >= 0 ? 'var(--primary)' : '#F6465D',
                              marginLeft: '6px',
      ...
      <div className="grid grid-cols-4 gap-2">
        <div className="p-3 rounded-lg text-center"
          style={{ background: 'var(--primary-bg, 0.05)', border: '1px solid var(--primary-bg, 0.1)' }}>
          <div className="text-[10px] uppercase tracking-wider mb-1" style={{ color: '#9CA3AF' }}>
            {t('leader', language)}
          </div>
          <div className="text-sm font-bold truncate" style={{ color: 'var(--primary)' }}>
            {leader?.trader_name || '-'}
          </div>
        </div>
        <div className="p-3 rounded-lg text-center" style={{ background: 'rgba(14, 203, 129, 0.05)' }}>
          ...
          <div className="text-sm font-bold mono"
            style={{ color: (leader?.currentPnl || 0) >= 0 ? 'var(--primary)' : '#F6465D' }}>
        ...
        <div className="p-3 rounded-lg text-center" style={{ background: 'rgba(139, 92, 246, 0.05)' }}>
          ...
          <div className="text-sm font-bold mono" style={{ color: 'rgb(195, 245, 60)' }}>
            {displayData.length}
          </div>
        </div>
      </div>
